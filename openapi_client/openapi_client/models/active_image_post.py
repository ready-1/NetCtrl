...
class ActiveImagePost(BaseModel):
    """
    ActiveImagePost
    """  # noqa: E501

    label: StrictStr = Field(
        description="Set the activated image:   * `image1`   * `image2` "
    )
    ...
    def label_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(["image1", "image2"]):
            raise ValueError("must be one of enum values ('image1', 'image2')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({"label": obj.get("label")})
        return _obj
