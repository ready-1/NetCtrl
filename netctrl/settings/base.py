...
LOGGING = {
    ...
    'loggers': {
        'django': {
            ...
            'level': 'INFO',  # Default level for Django's internal logs
            'propagate': True,
        },
        'app': {
            'handlers': ['console', 'file'],
...
ALLOWED_HOSTS = os.getenv('ALLOWED_HOSTS', '').split(',') # or config('ALLOWED_HOSTS', default='', cast=lambda v: [s.strip() for s in v.split(',')])

BUG_QUEUE_FILE = Path(BASE_DIR / "data" / os.getenv("BUG_QUEUE_FILE")) if os.getenv("BUG_QUEUE_FILE") else BASE_DIR / "data" / "bug_queue.json"
...
# Project Apps
INSTALLED_APPS += ['accounts','bugtracker', 'net_core', 'net_frontend', 'wiki']
...
STATICFILES_DIRS = [
    BASE_DIR / "static"  # Ensure this matches your directory structure
]
...
# The absolute path to the directory where media will be stored.
